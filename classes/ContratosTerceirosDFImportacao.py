...
class ContratosTerceirosDFImportacao:
    ...
    def check_header(self) -> bool:
        cabecalho = self.modelo.retornaCabecalhoContratosTerceiros()
        for col in cabecalho:
            if not col in self.df.columns:
                raise Exception(f"Coluna ({col}) não foi encontrada no cabeçalho do arquivo de importação de contratos de terceiros")
        return True
    ...
    def check_full_dates(self, index):
        resultado = ''
        for campo in self.modelo.datasCompletasContratosTerceiros:
    ...
    def check_short_dates(self, index):
        resultado = ''
        for campo in self.modelo.datasAbreviadasContratosTerceiros:
            if campo in self.df.columns:
                valor = self.df.at[index, campo]
                if not pd.isnull(valor):
    ...
    def check_currency_values_br(self, index):
        resultado = ''
        for campo in self.modelo.camposMonetariosContratosTerceiros:
            if campo in self.df.columns:
                valor = self.df.at[index, campo]
                if not pd.isnull(valor):
