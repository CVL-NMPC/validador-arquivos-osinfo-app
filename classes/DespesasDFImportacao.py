...
class DespesasDFImportacao:
    ...
    def check_header(self) -> bool:
        cabecalho = self.modelo.retornaCabecalhoDespesas()
    ...
    def check_full_dates(self, index):
        resultado = ''
        for campo in self.modelo.datasCompletasDespesas:
    ...
    def check_currency_values_br(self, index):
        resultado = ''
        for campo in self.modelo.camposMonetariosDespesas:
            if campo in self.df.columns:
                valor = self.df.at[index, campo]
                if not pd.isnull(valor):
